...
#Region ;- Reference Hives. (Do NOT Indent these. Compiling WILL fail due to line length limits!) - (( This might not happen anymore -Meteor 2022
Global Const $EngineSettingsClearHive[109][181] = [ _
	...
	['[OnlineSubsystemEpic.OnlineSubsystemEpic]','LogLevel=600','ProductId=f71b1231985f48d1af3de723e0a6acdd','SandboxId=076207fa2b5c4803a636af606c3c28b7','ClientId=bf659e4ce1264fd9ad93c83eba001914','DeploymentId=e03ac5a2b3444159b50aded07f1ed69b'], _
	...
	['[Experimental]','bUnloadUIScenesOnTransition=True','ParticleDynamicDataRingBufferSizeMB=16','ParticleBulkDataCollapseThreshold=0.2','bSpecialFxCanUseEmitterPool=TRUE','bAllowDeferredParticleTicking=TRUE','bParticleSystemOuterGCOptimization=TRUE','bPassthroughAimArray=TRUE'], _
...
Global Const $SystemSettingsClearHive[35][246] = [ _
	...
	['[IniVersion]','0=1555521695.000000','1=1580232924.000000'], _
	['[SettingsCheck]','Initialized=True'''] _
]

Global Const $GameSettingsClearHive[52][149] = [ _
	...
	['[IniVersion]','0=1555521316.000000','1=1650368720.000000'], _
...
[	_
	[ _	;World
		...
		[ _ ;Best
			...
			"TEXTUREGROUP_ImageBasedReflection=(MinLODSize=256,MaxLODSize=4096,MaxLODSizeTexturePack=4096,LODBias=0,LODBiasTexturePack=0,MinMagFilter=Linear,MipFilter=Linear,MipGenSettings=TMGS_Blur5)", _
			""], _
			[ _ ;Very Low
	...
	;-- Home (Simple mode)
		;- Get available resolutions and format them properly and then sort the array.
			...
			Else
				...
				_ArrayDelete($AvailableResolutions,0) ;- Remove count.
				_ArraySort($AvailableResolutions,1,default,default,default,2) ;- Sort this mfer.
		...
		Global $MainGUIHomeSimpleLabelDirectX11 = GUICtrlCreateLabelTransparentBG("Use DirectX11",613,128,235,13)
			...
			;- Windows versions older than WIN_7 do not support DirectX11.
	...
	;-- Fixes
		...
		If not @Error Then
			...
			Local $Split = StringSplit($Int_Settings,"|")
				_ArrayDelete($Split,0) ;- Why does every array function also add the count, nobody uses it ffs.
...
#Region ;- Internal Functions
				...
				If not $Found Then
					...
					If not $Found Then
						...
						;- ---------- ---------- ---------- ---------- ----------
							...
							_ArrayDelete($Files,0) ;- Delete the array count.
		...
		If $ProgramHomeState = "Advanced" or $InitBool Then
			...
			Else
				$Int_Settings = "Low|" & _ ;- World Quality.
								...
								$GUI_UNCHECKED & "|" & _ ;- Static Decals.
								$GUI_UNCHECKED & "|" & _ ;- Dynamic Decals.
								$GUI_UNCHECKED & "|" & _ ;- Unbatched Decals.
	...
	Func Interal_VerifyAndFixConfiguration($FileReadArray,$Hive) ;- This function verifies and fixes any issues with a supplied configuration file. (Except non-sense variables)
		...
		;- Keyvalue = The value of a keyname within a group such as "Spack"
		;- [URL] < Group
		;- Game=Spack < Keyname=Keyvalue
					...
					If not $DidWeHandleThatOneAlready Then ;- Check whether duplicates exist in the Hive for that keyname.
						...
						If $DuplicateCount > 1 Then ;- Duplicates have been found!
							...
							For $Tarantel = 0 To uBound($KeyValuesInHive) - 1 Step 1
								...
								EndIf
							Next
						Else ;- This keyname does not seem to be a duplicate but we have to check whether it exists multiple times within the group. If it does, it would be useless and wasting space!
			...
			If $ProgramHomeState = "Simple" Then
				...
				;- ----- ----- ----- ----- -----
					...
					If $Quality > 2 Then ;- If lower than High.
						...
						;- Whoops, they're going to remove that setting because some "pro" started drama. (Rip me, never played with fog)
						;- This is commented out for your own safety. While they did say they'll remove it, we all know how Hi-Rez works. Sloppy.
						;- So, we just don't set it in the first place to avoid getting people banned for modifying a restricted setting.
						;- $Array = Internal_ApplyKey($Array,"bAllowFog=","False")
				...
				;- ----- ----- ----- ----- -----
					...
					;- If the shadow quality is lower than high we want to yeet some settings.
				...
				;- ----- ----- ----- ----- -----
					...
					Local $SplitC = StringSplit($RRead,"x")
						_ArrayDelete($SplitC,0) ;- Remove count.
	...
	Func Internal_CreateQuicklaunchBypass()
		...
		While True ;- Mouse input
			...
			If WinGetTitle("[active]") = $S_Title and @Error = 0 Then
				...
				If _IsPressed("01") Then
					While _IsPressed("01")
						Sleep(10) ;- Wait until they let go of mouse1! (This is cheap af!)
					WEnd
		...
		If IsDeclared("GUI_Quicklaunch_Bits") Then _
			GUIDelete($GUI_Quicklaunch_Bits)
	...
	If WinGetTitle("[active]") = $ProgramName and @Error = 0 Then
		...
		;------------------------------------------------------------------------------
			...
			If ($ProgramHomeHelpState = 1 and $MenuSelected = 1 and not $MainGUIHomeDiscoveryDrawn) or ($ProgramHomeHelpState = 1 and $MenuSelected == 2) Then ;- Optimizations.
				...
				If not $SupressHoverImage Then
					...
					ElseIf $ProgramHomeState = "Simple" Then
						...
						ElseIf InRange2D($MousePos,295,71,495,114) Then
							DisplayHoverImage(295,71,200,43,$MainResourcePath & "HelpText/Resolution.jpg",500,40,345,185) ;- Resolution.
					...
					ElseIf $ProgramHomeState = "Advanced" Then
						...
						ElseIf InRange2D($MousePos,295,116,495,159) Then
							DisplayHoverImage(295,116,200,43,$MainResourcePath & "HelpText/Resolution.jpg",500,40,345,185) ;- Resolution.
				...
				If $DisplayHoverBG = 0 and $HoverBGDrawn Then ;- Optimizations, yeet that BG outta here!
					GUICtrlSetPos($MainGUIHomeHelpBackground,-$MinWidth,-$MinHeight,1,1)
					$HoverBGDrawn = False
				ElseIf $DisplayHoverImage = 0 and $HoverImageDrawn Then ;- Optimizations, yeet that Image outta here!
					WinMove($HoverInfoGUI,$sEmpty,-$ScrW*2,-$ScrH*2,0,0)
					$HoverImageDrawn = False
				EndIf
